"""Digest eval scenarios, fed through the real digest_prompt."""
...
def scenario_ips(key: str) -> list[str]:
    """Every IP present in a scenario's input — the set a digest may legitimately cite."""
    s = SCENARIOS[key]
    ips = {e.ip for e in s.entries} | {f.ip for f in s.findings} | {e.host_ip for e in s.exposures}
    return sorted(ip for ip in ips if IP_RE.fullmatch(ip))
